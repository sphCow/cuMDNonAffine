...
__global__
void kernel_calc_epsilon(double4 *X_, 
												 double4 *Y_, 
												 long* pid, 
												 double4 *epsilon, 
												 long N) {
  ...
  //	printf("dbg eps:: pid = %d(%ld) \t eps=%lf %lf %lf %lf\n",my_pid,gtid,my_epsilon.x,my_epsilon.y,my_epsilon.z,my_epsilon.w);
  //}
}
...
void kernel_calc_chi2(double4  *r, 
  ...
  double4 my_nebz_r, my_r, my_ref_r, my_nebz_epsilon;
	...
	double4 my_f_hchi = make_double4(0.0,0.0,0.0,0.0);
  ...
  for(int n=0; n<num_nebz; n++) {
    ...
    my_nebz_gtid = pid2gtid[my_nebz_pid];
    my_nebz_r = r[my_nebz_gtid];
		//my_nebz_epsilon = epsilon[my_nebz_pid]; // THIS IS WRONG
		my_nebz_epsilon = epsilon[my_nebz_gtid]; // THIS IS THE CORRECT ONE
    ...
    my_chi2 += pow(chi_x, 2.0) + pow(chi_y, 2.0);
    
    my_f_hchi.x += 4*drx - 2*(1.0 + my_nebz_epsilon.x)*dRx - 2*(my_nebz_epsilon.y      )*dRy;
    my_f_hchi.y += 4*dry - 2*(      my_nebz_epsilon.z)*dRx - 2*(my_nebz_epsilon.w + 1.0)*dRy;
    
    //dbg
    //if(my_nebz_pid==469 || my_nebz_pid==4698 || my_nebz_pid==13988) {
    //	printf("dbg nebz_eps:: pid_me = %ld \t pid_nebz=%ld(%ld) \t eps=%lf %lf %lf %lf\n",my_pid, my_nebz_pid,my_nebz_gtid,my_nebz_epsilon.x,my_nebz_epsilon.y,my_nebz_epsilon.z,my_nebz_epsilon.w);
    //}
     
  }

  dev_chi2[gtid] = my_chi2;
  dev_chi2_sqr[gtid] = my_chi2*my_chi2;
  
  if(add_hchi_forces) {
  	dev_f[gtid].x += h*my_f_hchi.x;
  	dev_f[gtid].y += h*my_f_hchi.y;
  }
}
...
// [NEW] get reference lattice 
double4* NonAffine::get_ref_lattice() {
	return ref_r;
} 
...
void NonAffine::allocate() {
  ...
  if(use_device == GPU) {
    ...
    cudaMalloc((void **) &dev_chi2_sqr, N*sizeof(*dev_chi2_sqr));
...
void NonAffine::calc_chi2(double4 *r) { // r has to be sorted by pid
  ...
  if(use_device == GPU) {
    ...
    */
    ...
    thrust::device_ptr<double> dev_ptr_chi2_sqr = thrust::device_pointer_cast(dev_chi2_sqr);
    mean_chi2     = thrust::reduce(dev_ptr_chi2, dev_ptr_chi2 + N)/static_cast<double>(N);
    mean_chi2_sqr = thrust::reduce(dev_ptr_chi2_sqr, dev_ptr_chi2_sqr + N)/static_cast<double>(N);
